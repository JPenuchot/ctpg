...
#include "stdex/cbitset.hpp"
#include "stdex/cvector.hpp"
#include "stdex/cqueue.hpp"
...
template <size_t N> using str_table = const char *[N];
...
namespace meta {
template <typename T> struct contains_type {};

template <typename... T> struct unique_type_list : contains_type<T>... {
  using variant_type = std::variant<T...>;

  template <typename New> struct insert {
  ...
  template <typename New> typename insert<New>::type operator+(New);
};

template <typename... T> struct unique_types_variant {
  using unique_list =
      decltype((std::declval<unique_type_list<>>() + ... + std::declval<T>()));
...
template <typename... T> constexpr int sum_size = (0 + ... + sizeof(T));
...
template <size_t First, size_t... Rest> struct max {
  static const size_t value = std::max(First, max<Rest...>::value);
};

template <size_t X> struct max<X> { static const size_t value = X; };

template <size_t... X> constexpr size_t max_v = max<X...>::value;
...
namespace utils {
template <typename T, typename... Args> constexpr T construct_default(Args...) {
  return T();
}
...
constexpr bool is_printable(char c) { return c >= 0x20 && c <= 0x7e; }
...
constexpr bool is_dec_digit(char c) { return c >= '0' && c <= '9'; }
...
template <typename T> struct fake_table {
...
template <size_t> struct ignore {
  template <typename T> constexpr ignore(T &&) {}
};

template <size_t X, typename = std::make_index_sequence<X - 1>>
class element {};

template <size_t X, size_t... I> class element<X, std::index_sequence<I...>> {
public:
  template <typename First, typename... Rest>
  constexpr decltype(auto) operator()(ignore<I>..., First &&arg,
                                      Rest &&...) const {
...
public:
  constexpr val(T &&v) : v(std::forward<T>(v)) {}
...
template <typename T> val(T &&) -> val<std::decay_t<T>>;

template <typename T> class create {
public:
  template <typename... Args> constexpr auto operator()(Args &&...) const {
    return T{};
  }
...
template <std::size_t ContIdx = 1, std::size_t ArgIdx = 2,
          typename = std::make_index_sequence<std::min(ContIdx, ArgIdx) - 1>,
          typename = std::make_index_sequence<std::max(ContIdx, ArgIdx) -
                                              std::min(ContIdx, ArgIdx) - 1>,
          bool container_first = ContIdx<ArgIdx> struct emplace_back {};

template <std::size_t ContIdx, std::size_t ArgIdx, std::size_t... Skip1,
          std::size_t... Skip2>
struct emplace_back<ContIdx, ArgIdx, std::index_sequence<Skip1...>,
                    std::index_sequence<Skip2...>, true> {
  template <typename Container, typename Arg, typename... Rest>
  constexpr decltype(auto) operator()(ignore<Skip1>..., Container &&container,
                                      ignore<Skip2>..., Arg &&arg,
                                      Rest &&...) const {
...
template <std::size_t ContIdx, std::size_t ArgIdx, std::size_t... Skip1,
          std::size_t... Skip2>
struct emplace_back<ContIdx, ArgIdx, std::index_sequence<Skip1...>,
                    std::index_sequence<Skip2...>, false> {
  template <typename Container, typename Arg, typename... Rest>
  constexpr decltype(auto) operator()(ignore<Skip1>..., Arg &&arg,
                                      ignore<Skip2>..., Container &&container,
                                      Rest &&...) const {
    container.emplace_back(std::move(arg));
    return std::move(container);
  }
...
template <std::size_t ContIdx = 1, std::size_t ArgIdx = 2,
          ...
          typename = std::make_index_sequence<std::max(ContIdx, ArgIdx) -
                                              std::min(ContIdx, ArgIdx) - 1>,
          bool container_first = ContIdx<ArgIdx> struct push_back {};

template <std::size_t ContIdx, std::size_t ArgIdx, std::size_t... Skip1,
          std::size_t... Skip2>
struct push_back<ContIdx, ArgIdx, std::index_sequence<Skip1...>,
                 std::index_sequence<Skip2...>, true> {
  template <typename Container, typename Arg, typename... Rest>
  constexpr decltype(auto) operator()(ignore<Skip1>..., Container &&container,
                                      ignore<Skip2>..., const Arg &arg,
                                      Rest &&...) const {
...
struct push_back<ContIdx, ArgIdx, std::index_sequence<Skip1...>,
                 std::index_sequence<Skip2...>, false> {
  template <typename Container, typename Arg, typename... Rest>
  constexpr decltype(auto) operator()(ignore<Skip1>..., const Arg &arg,
                                      ignore<Skip2>..., Container &&container,
                                      Rest &&...) const {
    container.push_back(arg);
    return std::move(container);
...
namespace buffers {
template <size_t N> class cstring_buffer {
public:
  template <size_t N1> constexpr cstring_buffer(const char (&source)[N1]) {
  ...
  struct iterator {
    ...
    constexpr iterator &operator+=(size_t len) {
      ptr += len;
      return *this;
    }
  };
  ...
  constexpr std::string_view get_view(iterator start, iterator end) const {
    return std::string_view(start.ptr, end.ptr - start.ptr);
  }
...
template <size_t N> cstring_buffer(const char (&)[N]) -> cstring_buffer<N>;

class string_buffer {
public:
  string_buffer(std::string &&str) : str(std::move(str)) {}

  string_buffer(const char *str) : str(str) {}
...
template <typename Buffer> using iterator_t = typename Buffer::iterator;
} // namespace buffers

template <typename ValueType> class nterm {
public:
  using value_type = ValueType;

  constexpr nterm(const char *name) : name(name) {
    if (name[0] == 0)
...
template <typename VT> class term_value {
public:
  constexpr term_value(VT v, source_point sp) : value(v), sp{sp} {}
...
public:
  ...
  constexpr char_term(char c, int precedence = 0,
                      associativity a = associativity::no_assoc)
      : term(precedence, a), c(c) {
    utils::copy_array(id, utils::c_names.name(c),
                      std::make_index_sequence<utils::char_names::name_size>{});
  }
...
template <size_t DataSize> class string_term : public term {
public:
  using internal_value_type = std::string_view;
  static const size_t dfa_size = (DataSize - 1) * 2;
  static const bool is_trivial = true;

  constexpr string_term(const char (&str)[DataSize], int precedence = 0,
                        associativity a = associativity::no_assoc)
      : term(precedence, a) {
...
template <typename Term, typename Ftor> class typed_term {
public:
  ...
  constexpr typed_term(Term t, Ftor f) : term(t), ftor(f) {}
  ...
  constexpr associativity get_associativity() const {
    return term.get_associativity();
  }
...
template <typename Ftor> class custom_term : public term {
public:
  using internal_value_type = std::invoke_result_t<Ftor, std::string_view>;
  static const size_t dfa_size = 0;
  static const bool is_trivial = false;

  constexpr custom_term(const char *custom_name, Ftor ftor, int precedence = 0,
                        associativity a = associativity::no_assoc)
...
template <typename ValueType> struct value_type<nterm<ValueType>> {
...
template <> struct value_type<error_recovery_token> { using type = no_type; };

template <typename T> using value_type_t = typename value_type<T>::type;

struct parse_options {
  constexpr parse_options &set_verbose(bool val = true) {
    verbose = val;
    return *this;
  }
  constexpr parse_options &set_skip_whitespace(bool val = true) {
    skip_whitespace = val;
    return *this;
  }
...
struct match_options {
  bool verbose = false;
  constexpr match_options &set_verbose(bool val = true) {
    verbose = val;
    return *this;
  }
};
...
namespace detail {
struct no_stream {
  template <typename T> constexpr const no_stream &operator<<(T &&) const {
    return *this;
  }
};
} // namespace detail

template <typename Iterator> struct recognized_term {
  Iterator it;
  size16_t term_idx;
};
...
struct dfa_state {
  ...
  conflicted_terms conflicted_recognition = {uninitialized16, uninitialized16,
                                             uninitialized16, uninitialized16};
  static const size_t transitions_size = meta::distinct_values_count<char>;
  size16_t transitions[transitions_size] = {};
};

struct char_range {
  constexpr char_range(char c) : start(c), end(c) {}

  constexpr char_range(char c1, char c2) : start(c1), end(c2) {}
  char start;
  char end;
};
...
public:
  ...
  template <size_t N> constexpr char_subset(const char (&str)[N]) {
    for (size_t i = 0; i < N - 1; ++i) // ignore trailing 0, so N - 1
      data.set(utils::char_to_idx(str[i]));
  }

  constexpr char_subset(char_range r) { add_range(r); }

  constexpr char_subset &add_range(char_range r) {
    for (size_t i = utils::char_to_idx(r.start); i <= utils::char_to_idx(r.end);
         ++i)
      data.set(i);
    return *this;
  }
  ...
  constexpr char_subset &set() {
    data.set();
    return *this;
  }
  constexpr char_subset &flip() {
    data.flip();
    return *this;
  }
  constexpr char_subset &set(size_t idx) {
    data.set(idx);
    return *this;
  }
...
template <size_t N> using dfa = stdex::cvector<dfa_state, N>;
...
private:
  constexpr slice prim() {
    auto old = size;
    size += 2;
    return slice{old, 2};
  }
  constexpr slice add(slice s1, slice s2) {
...
template <size_t N> class dfa_builder {
public:
  constexpr dfa_builder(dfa<N> &sm) : sm(sm) {}
...
private:
  constexpr void merge(size_t to, size_t from, bool keep_end_state = false,
                       bool mark_from_as_unreachable = false) {
    if (to == from)
      return;
...
template <size_t N> struct regex_pattern_data { const char (&pattern)[N]; };

template <size_t N, typename Parser>
constexpr void add_term_data_to_dfa(char c, dfa_builder<N> &b, const Parser &,
                                    size16_t idx) {
  using slice = utils::slice;
...
constexpr void add_term_data_to_dfa(const char (&str)[DataSize],
                                    dfa_builder<N> &b, const Parser &,
                                    size16_t idx) {
  using slice = utils::slice;
  slice prev{0, size32_t(b.size())};
...
constexpr void
add_term_data_to_dfa(const regex_pattern_data<PatternSize> &pattern_data,
                     dfa_builder<N> &b, const Parser &p, size16_t idx) {
  using slice = utils::slice;
  detail::no_stream s{};
  std::optional<slice> res =
      p.parse(parse_options{}.set_skip_whitespace(false),
              buffers::cstring_buffer(pattern_data.pattern), s);
...
constexpr auto dfa_match(const dfa<N> &sm, match_options options,
                         source_point sp, Iterator start, Iterator end,
  ...
  while (true) {
    ...
    if (options.verbose) {
      error_stream << sp << " REGEX MATCH: Current char "
                   << utils::c_names.name(*start) << "\n";
...
constexpr void write_dfa_state_diag_str(const dfa_state &st, Stream &s,
                                        size16_t idx,
                                        const StrTable &term_names) {
  s << "STATE " << idx;
  if (st.unreachable) {
    s << " (unreachable) \n";
    return;
  ...
  for (size_t i = 0; i < dfa_state::transitions_size + 1; ++i) {
    size16_t to = (i == dfa_state::transitions_size ? uninitialized16
                                                    : st.transitions[i]);
...
template <typename Builder> constexpr auto create_regex_parser(Builder &b);
} // namespace regex

namespace detail {
template <typename Arg> constexpr decltype(auto) make_term(Arg &&arg) {
  return std::forward<Arg>(arg);
}

constexpr auto make_term(char c) { return char_term(c); }

template <size_t N> constexpr auto make_term(const char (&str)[N]) {
  return string_term<N>(str);
}

template <typename ValueType> struct fake_root {
  using value_type = ValueType;
...
template <typename F, typename L, typename... R> class rule {
public:
  using f_type = F;
  ...
  template <typename F1> constexpr auto operator>=(F1 &&f) {
...
template <typename Arg> constexpr auto make_rule_item(Arg &&arg) {
...
template <typename ValueType>
constexpr auto
fake_root<ValueType>::operator()(const nterm<ValueType> &nt) const {
...
constexpr auto nterm<ValueType>::operator()(Args &&...args) const {
  return detail::rule(*this, std::make_tuple(detail::make_rule_item(args)...));
}
...
template <typename CursorStack, typename ValueStack, typename ErrorStream,
          typename Iterator>
struct parse_state {
  constexpr parse_state(CursorStack &cursor_stack, ValueStack &value_stack,
                        ErrorStream &error_stream, parse_options options,
                        Iterator buffer_begin, Iterator buffer_end)
      : cursor_stack(cursor_stack), value_stack(value_stack),
        error_stream(error_stream), options(options), current_sp{1, 1},
        current_it(buffer_begin), current_term_end_it(buffer_begin),
        buffer_end(buffer_end), current_term_idx(uninitialized16),
        recovery_mode(false), consume_mode(false) {
...
struct parse_table_cursor_stack_type<buffers::cstring_buffer<N>,
                                     EmptyRulesCount> {
  using type = stdex::cvector<size16_t, N + EmptyRulesCount + 1>;
};

template <typename Buffer, size_t EmptyRulesCount>
using parse_table_cursor_stack_type_t =
    typename parse_table_cursor_stack_type<Buffer, EmptyRulesCount>::type;
...
struct parser_value_stack_type<
    buffers::cstring_buffer<N>, EmptyRulesCount, ValueVariantType,
...
template <typename Buffer, size_t EmptyRulesCount, typename ValueVariantType>
using parser_value_stack_type_t =
    typename parser_value_stack_type<Buffer, EmptyRulesCount,
                                     ValueVariantType>::type;
} // namespace detail
...
template <typename Lexer> struct use_lexer { using type = Lexer; };
...
template <typename RootValueType, typename... Terms, typename... NTerms,
          typename... Rules, typename LexerUsage>
class parser<nterm<RootValueType>, std::tuple<Terms...>, std::tuple<NTerms...>,
             std::tuple<Rules...>, LexerUsage> {
  ...
  static const bool generate_lexer =
      std::is_same_v<LexerUsage, use_generated_lexer>;
  using lexer_type = typename LexerUsage::type;

public:
  constexpr parser(root_nterm_type grammar_root, term_tuple_type terms,
                   nterm_tuple_type nterms, rule_tuple_type &&rules, LexerUsage)
      : parser(grammar_root, terms, nterms, std::move(rules)) {}

  constexpr parser(root_nterm_type grammar_root, term_tuple_type terms,
                   nterm_tuple_type nterms, rule_tuple_type &&rules)
      : term_tuple(terms), nterm_tuple(nterms), rule_tuple(std::move(rules)) {
    auto seq_for_terms =
        std::make_index_sequence<std::tuple_size_v<term_tuple_type>>{};
    analyze_nterms(
        std::make_index_sequence<std::tuple_size_v<nterm_tuple_type>>{});
    analyze_nterm(detail::fake_root<value_type_t<root_nterm_type>>{});
    analyze_terms(seq_for_terms);
    analyze_eof();
    analyze_error_recovery_token();
    analyze_rules(
        std::make_index_sequence<std::tuple_size_v<rule_tuple_type>>{},
        grammar_root);
    analyze_states();
  ...
  constexpr std::optional<root_value_type>
  parse(const Buffer &buffer, ErrorStream &error_stream) const {
    return parse(parse_options{}, buffer, error_stream);
  }

  template <typename Buffer, typename ErrorStream>
  constexpr std::optional<root_value_type>
  parse(parse_options options, const Buffer &buffer,
        ErrorStream &error_stream) const {
    detail::parser_value_stack_type_t<Buffer, empty_rules_count,
                                      value_variant_type>
        value_stack{};
    detail::parse_table_cursor_stack_type_t<Buffer, empty_rules_count>
        cursor_stack{};

    detail::parse_state ps(cursor_stack, value_stack, error_stream, options,
    ...
    while (true) {
      ...
      if (entry.kind == parse_table_entry_kind::shift) {
        shift(ps, buffer.get_view(ps.current_it, ps.current_term_end_it), t_idx,
              entry.shift);
        consume_term(ps);
      } else if (entry.kind ==
                 parse_table_entry_kind::shift_error_recovery_token) {
  ...
  template <typename Stream> constexpr void write_diag_str(Stream &s) const {
    s << "PARSER"
      << "\n"
      << "\n";
...
private:
  ...
  static const size_t situation_address_space_size =
      rule_count * situation_size * term_count;
  static const size_t max_states =
      detail::deduce_max_states::template value<term_count, Rules::n..., 1>;
  static const size_t lexer_dfa_size =
      generate_lexer ? (0 + ... + Terms::dfa_size) : 1;

  using value_variant_type = meta::unique_types_variant_t<
      std::nullptr_t, no_type, value_type_t<NTerms>..., value_type_t<Terms>...>;
  ...
  using situation_queue_t =
      stdex::cqueue<situation_queue_entry, max_states * max_states>;

  struct symbol {
    constexpr symbol() : term(false), idx(uninitialized16) {}

    constexpr symbol(bool term, size16_t idx) : term(term), idx(idx) {}
  ...
  enum class parse_table_entry_kind : size16_t {
    error,
    success,
    shift,
    shift_error_recovery_token,
    reduce,
    rr_conflict
  };
  ...
  struct parse_table_entry {
    ...
    constexpr void set_shift(size16_t value) {
      shift = value;
      has_shift = true;
    }
    constexpr void set_reduce(size16_t value) {
      reduce = value;
      has_reduce = true;
    }
  };
  ...
  template <typename Term> constexpr auto make_symbol(const Term &t) const {
  ...
  template <size_t... I>
  constexpr void analyze_rules(std::index_sequence<I...>,
                               const root_nterm_type &root) {
    (void(analyze_rule<I>(std::get<I>(rule_tuple),
                          std::make_index_sequence<Rules::n>{})),
     ...);
    analyze_rule<root_rule_idx>(
        detail::fake_root<value_type_t<root_nterm_type>>{}(root),
        std::index_sequence<0>{});
    stdex::sort(rule_infos, [](const auto &ri1, const auto &ri2) {
      return ri1.l_idx < ri2.l_idx;
    });
  ...
  constexpr size16_t calculate_rule_last_term(size16_t rule_idx,
                                              size16_t rule_size) const {
  ...
  constexpr int calculate_rule_precedence(int precedence,
                                          size16_t rule_idx) const {
    if (precedence != 0)
      return precedence;
  ...
  constexpr associativity
  calculate_rule_associativity(size16_t rule_idx) const {
    size16_t last_term_idx = rule_last_terms[rule_idx];
    if (last_term_idx != uninitialized16)
  ...
  constexpr void analyze_rule(const detail::rule<F, L, R...> &r,
                              std::index_sequence<I...>) {
    size16_t l_idx =
        size16_t(utils::find_str(nterm_names, r.get_l().get_name()));
    ...
    if constexpr (Nr != root_rule_idx) {
      value_reductors[Nr] =
          &reduce_value<Nr, F, value_type_t<L>, value_type_t<R>...>;
    }
  }

  struct state_analyze_info {
    situation_queue_t situation_queue = {};
    stdex::cvector<size32_t, situation_address_space_size>
        closures[situation_address_space_size] = {};
  ...
  constexpr size32_t make_situation_idx(situation_info a) const {
    return a.rule_info_idx * situation_size * term_count +
           a.after * term_count + a.t;
  }
  ...
  constexpr const term_subset &
  make_right_side_slice_first(const rule_info &ri, size_t start,
  ...
  constexpr const term_subset &make_nterm_first(size16_t nt,
    ...
    for (size_t i = 0u; i < s.n; ++i) {
      ...
      add_term_subset(sai.nterm_first[nt],
                      make_right_side_slice_first(ri, 0, sai));
    }
    return sai.nterm_first[nt];
  }
  ...
  constexpr const term_subset &
  make_situation_first_after(const situation_info &addr, size32_t idx,
                             state_analyze_info &sai) {
    ...
    add_term_subset(sai.situation_first_after[idx],
                    make_right_side_slice_first(ri, addr.after + 1, sai));
    ...
    if (!s.term) {
      ...
      for (auto i = 0u; i < sl.n; ++i) {
        for (size16_t t = 0; t < term_count; ++t) {
          if (first.test(t)) {
            size32_t new_s_idx = make_situation_idx(
                situation_info{size16_t(sl.start + i), 0, t});
  ...
  constexpr auto solve_conflict(size16_t rule_info_idx,
                                size16_t term_idx) const {
  ...
  constexpr void situation_transition(size16_t state_idx, size32_t idx,
    ...
    size16_t symbol_idx =
        reduction ? get_parse_table_idx(true, addr.t) : s.get_parse_table_idx();
    ...
    situation_info new_addr =
        situation_info{addr.rule_info_idx, size16_t(addr.after + 1), addr.t};
    ...
    if (entry.kind == parse_table_entry_kind::shift &&
        s.idx == error_recovery_token_idx)
  ...
  constexpr void write_state_diag_str(Stream &s, size16_t idx) const {
    ...
    for (size_t i = nterm_count; i < nterm_count + term_count; ++i) {
      ...
      if (entry.kind == parse_table_entry_kind::success)
        ...
        s << " shift to " << entry.shift << " S/R CONFLICT, prefer reduce("
          << rule_infos[entry.reduce].r_idx << ") over shift\n";
      else if (is_shift(entry.kind) && entry.has_reduce)
        s << " shift to " << entry.shift
          << " S/R CONFLICT, prefer shift over reduce("
          << rule_infos[entry.reduce].r_idx << ")\n";
  ...
  constexpr static value_variant_type
  string_view_to_term_value(const term_tuple_type &term_tuple,
                            const std::string_view &sv, source_point sp) {
    const auto &t = std::get<TermIdx>(term_tuple);
    using term_value_type =
        value_type_t<std::tuple_element_t<TermIdx, term_tuple_type>>;
    return value_variant_type(term_value_type(t.get_ftor()(sv), sp));
  }

  template <typename F, typename LValueType, typename... RValueType,
            size_t... I>
  constexpr static LValueType reduce_value_impl(const F &f,
                                                value_variant_type *start,
                                                std::index_sequence<I...>) {
  ...
  template <size_t RuleIdx, typename F, typename LValueType,
            typename... RValueType>
  constexpr static value_variant_type reduce_value(const rule_tuple_type &rules,
                                                   value_variant_type *start) {
    return value_variant_type(reduce_value_impl<F, LValueType, RValueType...>(
        std::get<RuleIdx>(rules).get_f(), start,
        std::index_sequence_for<RValueType...>{}));
  }

  template <typename ParseState>
  constexpr void shift_recovery_token(ParseState &ps,
                                      size16_t new_cursor_value) const {
    if (ps.options.verbose)
      ps.error_stream << ps.current_sp << " PARSE: Shift to "
                      << new_cursor_value
  ...
  template <typename ParseState>
  constexpr void shift(ParseState &ps, const std::string_view &sv,
                       size16_t term_idx, size16_t new_cursor_value) const {
    if (ps.options.verbose)
      ps.error_stream << ps.current_sp << " PARSE: Shift to "
                      << new_cursor_value << ", term: " << sv << "\n";
    ps.cursor_stack.push_back(new_cursor_value);
  ...
  constexpr void reduce(ParseState &ps, size16_t rule_info_idx) const {
    ...
    if (ps.options.verbose) {
      ps.error_stream << ps.current_sp << " PARSE: Reduced using rule "
                      << ri.r_idx << "  ";
      write_rule_diag_str(ps.error_stream, rule_info_idx);
      ps.error_stream << "\n";
    }

    ps.cursor_stack.erase(ps.cursor_stack.end() - ri.r_elements,
                          ps.cursor_stack.end());
    size16_t new_cursor_value =
        parse_table[ps.cursor_stack.back()][ri.l_idx].shift;
    ...
    ps.cursor_stack.push_back(new_cursor_value);
    value_variant_type *start =
        ps.value_stack.data() + ps.value_stack.size() - ri.r_elements;
    value_variant_type lvalue(value_reductors[ri.r_idx](rule_tuple, start));
    ps.value_stack.erase(ps.value_stack.end() - ri.r_elements,
                         ps.value_stack.end());
  ...
  constexpr bool pop_stacks(ParseState &ps) const {
    ...
    if (ps.cursor_stack.size() == 0) {
      if (ps.options.verbose) {
        ps.error_stream << ps.current_sp
                        << " PARSE: Could not recover from error \n";
      }
      return false;
    }

    if (ps.options.verbose) {
      ps.error_stream << ps.current_sp << " PARSE: Recovering to state "
                      << ps.cursor_stack.back() << "\n";
    }
  ...
  constexpr bool consume_term_recovering(ParseState &ps) const {
    ...
    if (ps.options.verbose) {
      ps.error_stream << ps.current_sp << " PARSE: Recovery, consuming term "
                      << term_names[ps.current_term_idx] << " \n";
    }
    consume_term(ps);
    return true;
  }
  ...
  constexpr size16_t get_current_term(ParseState &ps) const {
    ...
    if constexpr (generate_lexer) {
      res = regex::dfa_match(lexer_sm, opts, ps.current_sp, ps.current_it,
                             ps.buffer_end, ps.error_stream);
    } else {
      lexer_type custom_lexer;
      res = custom_lexer.match(opts, ps.current_sp, ps.current_it,
                               ps.buffer_end, ps.error_stream);
    }
  ...
  constexpr void create_lexer(std::index_sequence<I...>) {
    if constexpr (generate_lexer) {
      ...
      constexpr bool trivial_lexer =
          (true && ... && std::tuple_element_t<I, term_tuple_type>::is_trivial);
      if constexpr (trivial_lexer)
        (void(regex::add_term_data_to_dfa(std::get<I>(term_tuple).get_data(), b,
                                          no_parser{}, size16_t(I))),
         ...);
      else {
  ...
  using value_reductor = value_variant_type (*)(const rule_tuple_type &,
                                                value_variant_type *);
  value_reductor value_reductors[rule_count] = {};

  using string_view_to_term_value_t = value_variant_type (*)(
      const term_tuple_type &, const std::string_view &, source_point);
...
parser(Root, Terms, NTerms, Rules &&)
    -> parser<Root, Terms, NTerms, Rules, use_generated_lexer>;

template <typename Root, typename Terms, typename NTerms, typename Rules,
          typename LexerUsage>
parser(Root, Terms, NTerms, Rules &&, LexerUsage)
    -> parser<Root, Terms, NTerms, Rules, LexerUsage>;

template <typename... Terms> constexpr auto terms(const Terms &...terms) {
  return std::make_tuple(detail::make_term(terms)...);
}

template <typename... NTerms> constexpr auto nterms(NTerms... nterms) {
  return std::make_tuple(nterms...);
}

template <typename... Rules> constexpr auto rules(Rules &&...rules) {
  return std::make_tuple(std::move(rules)...);
}

struct skip {
  template <typename T> constexpr skip(T &&) {}
};
...
public:
  ...
  constexpr auto match(match_options options, source_point sp, Iterator start,
                       Iterator end, ErrorStream &error_stream) {
    if (start == end)
...
private:
  ...
  template <typename Iterator, typename ErrorStream>
  constexpr auto recognized(size16_t idx, size_t len, match_options options,
                            source_point sp, Iterator start,
                            ErrorStream &error_stream) {
...
template <typename Builder> constexpr auto create_regex_parser(Builder &b) {
  ...
  constexpr custom_term regex_digit_09(
      "regex_digit_09", [](auto sv) { return size32_t(sv[0]) - '0'; });
  ...
  return parser(
      ...
      rules(
          ...
          number(number, regex_digit_09) >=
              [](size32_t n, size32_t x) { return n * 10 + x; },
          primary(regex_digit_09) >=
              [&b](size32_t number) {
                return b.primary_char(char(number + '0'));
              },
          primary(regex_primary) >=
              [&b](const auto &s) { return b.primary_subset(s); },
          primary('(', expr, ')') >= _e2, q_expr(primary),
          ...
          q_expr(primary, '{', number, '}') >=
              [&b](slice s, skip, size32_t n, skip) { return b.rep(s, n); },
          concat(q_expr),
          concat(concat, q_expr) >=
              [&b](slice s1, slice s2) { return b.cat(s1, s2); },
          alt(concat),
          alt(alt, '|', alt) >=
              [&b](slice s1, skip, slice s2) { return b.alt(s1, s2); },
          expr(alt)),
...
constexpr size32_t analyze_dfa_size(const char (&pattern)[N]) {
  ...
  detail::no_stream s{};
  auto res = p.parse(parse_options{}.set_skip_whitespace(false), buffer, s);
  if (!res.has_value())
...
template <auto &Pattern> class expr {
public:
  ...
  constexpr expr() {
    ...
    auto s = p.parse(parse_options{}.set_skip_whitespace(false),
                     buffers::cstring_buffer(Pattern), stream);
    if (!s.has_value())
      throw std::runtime_error("invalid regex");
    b.mark_end_states(s.value(), 0);
  }

  template <typename Stream> constexpr void debug_parse(Stream &s) const {
    regex::dfa_size_analyzer a;
    auto p = regex::create_regex_parser(a);
    p.parse(parse_options{}.set_skip_whitespace(false).set_verbose(),
            buffers::cstring_buffer(Pattern), s);
  }

  template <size_t N> constexpr bool match(const char (&str)[N]) const {
    return match(buffers::cstring_buffer(str));
  }

  template <typename Buffer> constexpr bool match(const Buffer &buf) const {
    detail::no_stream s;
    return match(buf, s);
  }
  ...
  constexpr bool match(match_options opts, const Buffer &buf, Stream &s) const {
    ...
    if (res.term_idx == 0 && res.it == buf.end())
      ...
      if (res.term_idx == 0)
        s << "Leftover text after recognition: "
          << buf.get_view(res.it, buf.end()) << "\n";
      else
...
template <auto &Pattern> class regex_term : public term {
public:
  using internal_value_type = std::string_view;
  ...
  constexpr regex_term(int precedence = 0,
                       associativity a = associativity::no_assoc)
      : regex_term(nullptr, precedence, a) {}

  constexpr regex_term(const char *custom_name, int precedence = 0,
                       associativity a = associativity::no_assoc)
      : term(precedence, a), custom_name(custom_name) {
    id[0] = 'r';
    id[1] = '_';
    utils::copy_array(&id[2], Pattern,
                      std::make_index_sequence<pattern_size>{});
  }

  constexpr const char *get_name() const {
    return custom_name ? custom_name : id;
  }
  constexpr const char *get_id() const { return id; }
  constexpr auto get_data() const {
    return regex::regex_pattern_data<pattern_size>{Pattern};
  }
